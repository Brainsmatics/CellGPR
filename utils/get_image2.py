...
class get_matching_results:
    ...
    def get_array(self, z, size_y, size_x, inv_mat, resolution):
        # print('task {} start!'.format(z))
        image_array =  np.zeros([size_y, size_x], dtype='uint16')
        for y in range(size_y):
            for x in range(size_x):
                p = np.array([[x * self.scale], [y * self.scale], [z], [1]])
                index = inv_mat.dot(p)
                x0, y0, z0 = int(index[0, 0] / resolution[0]), int(index[1, 0] / resolution[0]), int(
                    index[2, 0] / resolution[1])
                # print(z0, y0, x0)
                try:
                    image_array[y, x] = self.fMOST_array[z0, y0, x0]
                except:
                    pass
        # print('task {} done!'.format(z))
        return z, image_array
        # return z
    ...
    def worker(self, input, output):
        ...
        for z, size_y, size_x, inv_mat, resolution in iter(input.get, 'STOP'):
            result = self.get_array(z, size_y, size_x, inv_mat, resolution)
            # result = self.func(z)
            output.put(result)
            # print(output.get())


    def get_image_array(self, two_photon_image, fMOST_image, inv_mat, thickness, scale = 0.8,resolution = [0.65,2]):
        ...
        tasks = [(z, size_y, size_x, inv_mat, resolution) for z in range(size_z)]

        task_queue = Queue()
        result_queue = Queue()
        # max_num = multiprocessing.cpu_count()
        max_num = 20
        for task in tasks:
            task_queue.put(task)
        ...
        for i in range(len(tasks)):
            # print('*' * 100)
            z, image_array0 = result_queue.get()
            image_array[z] = image_array0
    ...
    def run(self):
        ...
        image_array = self.get_image_array(self.two_photon_image, self.fMOST_image, inv_mat, self.thinckness, self.scale)
...
if __name__ == '__main__':
    ...
    a = get_matching_results(fMOST_data, two_photon_data, fMOST_image, two_photon_image, thinkness, output_path, scale)
    a.run()